...
let point_pair_to_recd (pp : point_pair) : point_recd =
  let (ppx, ppy) = pp in {x = ppx; y = ppy} ;;
...
let point_recd_to_pair (pr : point_recd) : point_pair = 
   let prx, pry = pr.x, pr.y in (prx, pry);;
...
let rec partition (f : 'a -> bool) (lst : 'a list) : ('a list * 'a list) =
  ...
  | [] -> ([],[])
  | head :: tail ->
    let (ttrue, ffalse) = partition f tail in 
    if f head then (head :: ttrue, ffalse)
    else (ttrue, head :: ffalse) ;;
